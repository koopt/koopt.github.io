	...
	var trigger=0, name, age, sex, education, mathematic, asu, cutting, programming; // обьявляем переменные которые понадобятся в ходе работы
		...
		function test(){ // Функция которая опрашивает пользователя
			...
			team = confirm("Вы умеете работать в коллективе?");
		...
		function writeToPage(){ // функция которая записывает значение из наших переменных в элементы на странице
			...
			*/
			...
			document.getElementById("team").checked = team;
		...
		if (trigger==1) {
		if (( document.getElementById("programming").checked ==true && document.getElementById("mathematic").checked ==true ) || (document.getElementById("cutting").checked ==true && document.getElementById("education").checked ==true 
